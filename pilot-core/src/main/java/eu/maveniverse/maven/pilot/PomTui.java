...
public class PomTui {
    ...
    public record SnippetInfo(List<String> lines, String source) {}
    ...
    public PomTui(String rawPom, String effectivePom, String fileName) throws Exception {
    ...
    /**
     * Runs the POM viewer TUI.
     * @return true if the user requested to edit the POM file, false if they just quit.
     */
    ...
    boolean handleEvent(Event event, TuiRunner runner) {
        ...
        if (key.isCharIgnoreCase('o') && editSupported) {
            editRequested = true;
            runner.quit();
            return true;
        }
    ...
    private void renderXmlTree(Frame frame, Rect area) {
        ...
        int lineNumWidth = String.valueOf(visible.size()).length();
        ...
        for (int i = 0; i < visible.size(); i++) {
            ...
            List<Span> spans = new ArrayList<>();
            String num = String.format("%" + Math.max(lineNumWidth, 3) + "d", i + 1);
            spans.add(Span.raw(num + " \u2502 ").fg(Color.DARK_GRAY));
    ...
    private void renderFooter(Frame frame, Rect area, boolean detailVisible) {
        ...
        if (searchMode) {
            ...
            if (editSupported) {
                spans.add(Span.raw("o").bold());
                spans.add(Span.raw(":Edit  "));
            }
            spans.add(Span.raw("q").bold());
            spans.add(Span.raw(":Quit"));
        }
