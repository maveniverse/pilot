...
class PilotShell {
    ...
    interface ToolPanelFactory {
        ToolPanel create(String toolId, MavenProject project, List<MavenProject> scope) throws Exception;
    }
    ...
    private List<MavenProject> selectedScope;
    ...
    private final ExecutorService panelExecutor = Executors.newSingleThreadExecutor(r -> {
    ...
    PilotShell(ReactorModel reactorModel, List<MavenProject> projects, ToolPanelFactory panelFactory) {
    ...
    private boolean handleMouseEvent(MouseEvent mouse) {
        ...
        // Scroll follows keyboard focus
        if (mouse.isScroll()) {
            if (focus == Focus.TREE && treePane != null && treeArea != null) {
                treePane.handleMouseEvent(mouse, treeArea);
            } else if (focus == Focus.CONTENT && activePanel != null && contentArea != null) {
                activePanel.handleMouseEvent(mouse, contentArea);
            }
            return true;
        }
        ...
        String cacheKey = tool.isModuleIndependent()
                ...
                : tool.id + ":"
                        + (selectedProject != null
                                ? selectedProject.getGroupId() + ":" + selectedProject.getArtifactId()
                                : "null");
        try {
            ToolPanel panel = panelFactory.create(tool.id, selectedProject, selectedScope);
            if (panel != null) {
    ...
    private void refreshActivePanel() {
        ...
        // Already loading this exact panel
        if (cacheKey.equals(loadingCacheKey)) {
            return;
        }

        // Start async panel creation
        activePanel = null;
        loadingCacheKey = cacheKey;
        final MavenProject proj = selectedProject;
        final List<MavenProject> scope = selectedScope;
        final int subView = currentSubView;
        panelExecutor.submit(() -> {
            try {
                ToolPanel panel = panelFactory.create(tool.id, proj, scope);
    ...
    private void onModuleSelected(MavenProject project) {
    ...
    private List<MavenProject> computeScope() {
    ...
    private void renderInfoBar(Frame frame, Rect area) {
        ...
        if (focus == Focus.TREE && treePane != null) {
            ...
            if (selectedProject != null) {
                statusSpans.add(
                        Span.raw(" │ " + selectedProject.getArtifactId()).fg(theme.secondaryTextColor()));
            }
