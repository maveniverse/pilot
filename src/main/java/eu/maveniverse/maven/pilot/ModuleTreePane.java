...
class ModuleTreePane {
    ...
    private final Consumer<MavenProject> onSelectionChanged;
    ...
    ModuleTreePane(ReactorModel reactorModel, Consumer<MavenProject> onSelectionChanged) {
    ...
    List<MavenProject> selectedSubtree() {
        var visible = visibleNodes();
        Integer sel = tableState.selected();
        if (sel != null && sel >= 0 && sel < visible.size()) {
    ...
    private void notifySelection() {
        MavenProject project = selectedProject();
        if (project != null) {
    ...
    private void updateSearchMatches() {
        ...
        for (int i = 0; i < visible.size(); i++) {
            ...
            String searchable = node.project.getGroupId() + ":" + node.project.getArtifactId();
    ...
    private Row buildRow(ReactorModel.ModuleNode node, boolean showGa, int gaColWidth) {
        ...
        if (showGa) {
            String ga = compactGa(node.project.getGroupId(), node.project.getArtifactId(), gaColWidth);
